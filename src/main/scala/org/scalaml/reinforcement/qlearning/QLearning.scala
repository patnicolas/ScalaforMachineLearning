...
final class QLearning[T](
  ...
  private def train: Option[QLModel] = Try {
    ...
    val completions = Range(0, conf.numEpisodes).map(_ => if (train(-1)) 1 else 0).sum
   ...
   */
  private def train(s0: Int): Boolean = {
    ...
    val finalState = search(QLIndexedState(qlSpace.init(s0), 0))
